    ...
    protected void pushConnection(PooledConnection connection) throws SQLException {
        synchronized (state) {
            ...
            if (connection.isValid()) {
                ...
                logger.info("A bad connection (" + connection.getRealHashCode() + ") attempted to return to the pool," +
        ...
        while (conn == null) {
            synchronized (state) {
                ...
                if (!state.idleConnections.isEmpty()) {
                    ...
                    logger.info(String.format("Checked out connection %s from pool.", conn.getRealHashCode()));
                }
                // 如果无空闲连接，则创建新的连接
                else {
                    // 活跃连接数不足
                    if (state.activeConnections.size() < poolMaximumActiveConnections) {
                        ...
                        logger.info(String.format("Created connection %s .", conn.getRealHashCode()));
                    }
                    // 活跃连接数已满
                    else {
                        // 取得活跃连接列表中的第一个，也就是最老的一个连接
                        ...
                        // 如果校验时间不够长则等待
                            ...
                            try {
                                ...
                                logger.info(String.format("Waiting as long as %s milliseconds for connection.", poolTimeToWait));
                ...
                if (conn != null) {
                    if (conn.isValid()) {
                        ...
                        logger.info(String.format("A bad connection (%s) was returned from the pool, "
                            + "getting another connection", conn.getRealHashCode()));
                        // 如果没有获取统计信息，则为失败链接+1
                        state.badConnectionCount++;
